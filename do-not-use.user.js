...
// @version      2025-02-18
// @description  不知道是用来干什么的脚本。不要把第 7, 42 行的 DO_NOT_EDIT_ME 修改然后在 55 行加入一些内容然后运行这个脚本。
// @author       XiEn
...
(function() {
    ...
    //  '{"data":{"operationType":"1","studentCode":"?????????","electiveBatchCode":"0123456789abcdef0123456789abcdef","teachingClassId":"???????????????????","courseKind":"??","teachingClassType":"???"}}',
    //  '{"data":{"operationType":"1","studentCode":"?????????","electiveBatchCode":"0123456789abcdef0123456789abcdef","teachingClassId":"???????????????????","courseKind":"??","teachingClassType":"???"}}', // 行末记得加逗号

    //  首先 F12 打开控制台，切到“网络”选项卡
    //  然后在选课页面上操作选课（如果“选课”按钮点不了就右键-“检查元素”然后把 <a class="cv-choice sc-add cv-disabled" ...> 这里的 cv-disabled 删掉再点）
    //  正确操作的话会看到下面多了一条 volunteer.do 的记录，单击后打开右侧“负载”选项卡查看 addParam 一项
    //  然后复制到上面，用一对单引号包裹，保存即可。可参考上面的格式。
    ];
	...
	$(document).ready(function() {
		...
		console.log("[XiEn's registration script loaded]");
		showNotification("XiEn's registration script loaded 🚀", '#9A269B', 5000);
	});
})();
